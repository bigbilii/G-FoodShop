package me.guoxin.fp;

import java.util.*;
import java.util.Map.Entry;

public class Fptree {
    private static final float SUPPORT = 0.1f;
    private static long absSupport;

    public static Map<Set<FpNode>, Long> make(List<String[]> matrix ) {
     ...
     * @param header
     */
    private static Map<Set<FpNode>, Long> fpGrowth(FpNode root, Map<String, FpNode> header, String idName) {
        ...
        if (isSinglePath(header, firstIdName)) {// 只有一条路径时，求路径上的所有组合即可得到调节频繁集
            ...
            paths.add(leaf);
            FpNode node = leaf;
            while (node.getParent().getIdName() != null) {
                paths.add(node.getParent());
                node = node.getParent();
            }
            ...
            for (int i = keysArray.length - 1; i >= 0; i--) {// 递归求条件树的频繁集
                ...
                for (FpNode leaf : leafs) {
                    ...
                    FpNode node = leaf;
                    while (node.getParent().getIdName() != null) {
                        path.add(node.getParent().getIdName());
                        node = node.getParent();
                    }
                    leafCount += leaf.getCount();
                    if (path.size() > 0)
                        paths.put(path, leaf.getCount());
                    else {// 没有父结点
                        noParentNode = leaf;
                    }
                }
    ...
    private static Map<Set<FpNode>, Long> addLeafToFrequent(FpNode leaf,
        ...
        if (conditionFres.size() == 0) {
            ...
            set.add(leaf);
            conditionFres.put(set, leaf.getCount());
        } else {
    ...
    private static boolean isSinglePath(Map<String, FpNode> header,
                                        String tableLink) {
        if (header.size() == 1 && header.get(tableLink).getNext() == null)
            return true;
        return false;
    }
    ...
    private static Holder getConditionFpTree(Map<List<String>, Long> paths) {
        ...
        for (Entry<List<String>, Long> entry : paths.entrySet()) {
    ...
    private static Map<Set<FpNode>, Long> getCombinationPattern(
        ...
        for (int mask = 1; mask < (1 << size); mask++) {// 求所有组合，从1开始表示忽略空集
            ...
            for (FpNode node : set) {
                if (node.getCount() < minValue)
                    minValue = node.getCount();
            }
...
//				System.out.println(count);
    ...
    private static String[] getOrderLine(String[] line, Map<String, Integer> frequentMap) {
        ...
        List<Entry<String, Integer>> mapList = new ArrayList<Entry<String, Integer>>(
                countMap.entrySet());
        Collections.sort(mapList, new Comparator<Entry<String, Integer>>() {// 降序排序
            @Override
